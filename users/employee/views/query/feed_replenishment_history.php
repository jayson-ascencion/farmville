<?php
    ...
    $sql = "SELECT * FROM feedtransaction WHERE transactionType = 'replenishment' AND archive = 'not archived' ORDER BY feed_ID ASC";
    $stmt = $conn->query($sql);
    if($stmt){
        if($stmt->rowCount() > 0){
            echo '
            ...
            <table id="feedReplenishment" class="table table-sm responsive border table-hover text-center rounded rounded-3 overflow-hidden" style="width: 100%">';
                ...
                while($row = $stmt->fetch()){
                    ...
                    echo "<tr>";
                        ...
                        echo "<td>";
                        //update icon/button
                        echo '<a href="update_feed_replenishment.php?id='. $row['transaction_ID'] .'" class="m-1 text-decoration-none" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="Edit Record">
