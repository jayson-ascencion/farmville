<?php
    ...
    if($stmt){
        if($stmt->rowCount() > 0){
            echo '<table id="medicineReduction" class="table table-sm responsive border table-hover text-center rounded rounded-3 overflow-hidden" style="width: 100%">';
                ...
                while($row = $stmt->fetch()){
                   ...
                   $expDateFormatted = date("M. d, Y", $dataSortExpD);
                    ...
                    echo "<tr data-id='".$row['reduction_ID'] . "'>";
                        ...
                        echo '<a href="archive_medicine_reduction.php?id='. $row['reduction_ID'] .'" class="m-1 text-decoration-none" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="Archive Record">
