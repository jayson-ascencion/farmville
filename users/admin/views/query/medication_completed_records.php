<?php
    ...
    $sql = "SELECT * FROM schedules WHERE scheduleType = 'medication' AND status = 'completed' AND archive = 'not archived' ORDER BY administrationSched ASC";
    $stmt = $conn->query($sql);
    if($stmt){
        if($stmt->rowCount() > 0){
            echo '<table id="medicationSchedules" class="table table-sm  border responsive table-hover text-center rounded rounded-3 overflow-hidden" style="width: 100%">';
                ...
                while($row = $stmt->fetch()){
                    ...
                    echo "<tr>";
                        ...
                        echo "<td>";
                        //update icon/button
                        echo '<a href="update_medication_completed.php?id='. $row['administration_ID'] .'" class="m-1 text-decoration-none" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="Edit Record">
                        ...
                        //view button    
                        echo '<a href="view_completed_med.php?id='. $row['administration_ID'] .'" class="m-1 text-decoration-none" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="View Record">
                        ...
                        echo '<a href="archive_medication_completed.php?id='. $row['administration_ID'] .'" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="Archive Record">
            ...
            echo '<table id="medicationSchedules" class="tabletable-sm  border responsive table-hover text-center rounded rounded-3 overflow-hidden" style="width: 100%">';
            echo "<thead class='text-white' style='background-color: #2d4154'>";
                echo "<tr>";
                    echo "<th>Administration ID</th>";
                    echo "<th>Chicken Batch ID</th>";
