<?php
    ...
    // Select all medicine administration schedules where status is completed and order by administration schedule in ascending
    $sql = "SELECT * FROM schedules WHERE scheduleType = 'vaccination' AND status = 'completed' AND archive = 'not archived' AND administeredBy = $employee_ID ORDER BY administrationSched ASC";
    $stmt = $conn->query($sql);
    if($stmt){
        if($stmt->rowCount() > 0){
            echo '
           ...
           <table id="vaccinationSchedules_Employee" class="table border table-sm responsive table-hover text-center rounded rounded-3 overflow-hidden" style="width: 100%">';
           ...
           <table id="vaccinationSchedules_Employee" class="table border table-sm responsive table-hover text-center rounded rounded-3 overflow-hidden" style="width: 100%">';
                echo "<thead class='text-white' style='background-color: #DC143C'>";
                    echo "<tr>";
                        echo "<th>Administration hhh ID</th>";
                        echo "<th>Chicken Batch ID</th>";
                        echo "<th>Coop Number</th>";
                        echo "<th>Medicine Name</th>";
                        echo "<th>Vaccination Type</th>";
