<?php
    ...
    $sql = "SELECT * FROM medicinetransaction WHERE archive='not archived' AND transactionType = 'reduction' ORDER BY transactionDate ASC";
    $stmt = $conn->query($sql);
    if($stmt){
        if($stmt->rowCount() > 0){
            echo '
                ...
                while($row = $stmt->fetch()){
                   ...
                   $expDateFormatted = date("M. d, Y", $dataSortExpD);
                    ...
                    echo "<tr data-id='".$row['transaction_ID'] . "'>";
                        ...
                        echo "<td data-id='".$row['transaction_ID'] . "'>";
                        //update icon/button
                        
                        echo '<a href="update_medreduction_form.php?id='. $row['transaction_ID'] .'" class="m-1 text-decoration-none" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="Edit Record">
                        ...
                        echo '<a href="viewMedReduction.php?id='. $row['transaction_ID'] .'" class="m-1 text-decoration-none" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="View Record">
                        ...
                        echo '<a href="archive_medicine_reduction.php?id='. $row['transaction_ID'] .'" class="m-1 text-decoration-none" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="Archive">
