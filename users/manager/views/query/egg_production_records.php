<?php
    ...
    $sql = "SELECT * FROM eggproduction WHERE archive='not archived' ORDER BY eggBatch_ID ASC";
    $stmt = $conn->query($sql);
    if($stmt){
        if($stmt->rowCount() > 0){
            echo '<table id="eggProduction" class="table table-sm responsive border table-hover text-center rounded rounded-3 overflow-hidden" style="width: 100%">';
                ...
                while($row = $stmt->fetch()){
                    ...
                    $dataSortExpD = strtotime($row['collectionDate']);
                    //format the date
                   ...
                   // $days = $diff->format('%r%a');
                    ...
                    echo "<tr>";
                        ...
                        echo "<td>" . $row['collectionType'] . "</td>";
                        echo "<td data-sort='". $dataSortExpD ."'>" . $expDateFormatted . "</td>";
                        //echo "<td>" . $row['note'] . "</td>";
                        echo "<td>";
                        //update icon/button
                        echo '<a href="update_egg_form.php?id='. $row['eggBatch_ID'] .'" class="m-1 text-decoration-none" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="Edit Record">
                        ...
                        echo '<a href="viewEggProd.php?id='. $row['eggBatch_ID'] .'" class="m-1 text-decoration-none" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="View Record">
                        ...
                        echo '<a href="archive_egg.php?id='. $row['eggBatch_ID'] .'" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="Archive Record">
