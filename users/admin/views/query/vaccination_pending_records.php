<?php
    ...
    // Select all vaccination administration schedules where status is pending and order by ascending
    $sql = "SELECT * FROM schedules WHERE scheduleType = 'vaccination' AND status = 'pending' AND archive = 'not archived' ORDER BY administrationSched ASC";
    $stmt = $conn->query($sql);
    if($stmt){
        if($stmt->rowCount() > 0){
            echo '
           ...
           <table id="vaccinationSchedules" class="table table-sm border responsive table-hover text-center rounded rounded-3 overflow-hidden" style="width: 100%">';
                ...
                while($row = $stmt->fetch()){
                    ...
                    echo "<tr>";
                        ...
                        //update icon/button
                            ...
                            echo '<a href="archive_vaccination_pending.php?id='. $row['administration_ID'] .'" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="Delete">
