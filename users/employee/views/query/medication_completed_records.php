<?php
    ...
    $sql = "SELECT * FROM schedules WHERE scheduleType = 'medication' AND status = 'completed' AND archive = 'not archived' AND administeredBy = $employee_ID ORDER BY administrationSched ASC";
    $stmt = $conn->query($sql);
    if($stmt){
        if($stmt->rowCount() > 0){
            echo '
           ...
           <table id="medicationSchedules_Employee" class="table table-sm border responsive table-hover text-center rounded rounded-3 overflow-hidden" style="width: 100%">';
