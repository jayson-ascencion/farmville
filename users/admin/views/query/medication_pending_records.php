<?php
    ...
    $sql = "SELECT * FROM schedules WHERE scheduleType = 'medication' AND status = 'pending' AND archive = 'not archived' ORDER BY administrationSched ASC";
    $stmt = $conn->query($sql);
    if($stmt){
        if($stmt->rowCount() > 0){
            echo '<table id="medicationSchedules" class="table table-sm  border responsive table-hover text-center rounded rounded-3 overflow-hidden" style="width: 100%">';
                ...
                while($row = $stmt->fetch()){
                    ...
                    echo "<tr>";
                        ...
                        echo "<td>";
                        //update icon/button
                            echo '<a href="update_medication_pending.php?id='. $row['administration_ID'] .'" class="m-1 text-decoration-none" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="Edit Record">
                            ...
                            //view button
                            echo '<a href="view_pending_med.php?id='. $row['administration_ID'] .'" class="m-1 text-decoration-none" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="View Record">
                            ...
                            echo '<a href="archive_medication_pending.php?id='. $row['administration_ID'] .'" data-bs-toggle="tooltip" data-bs-placement="left" data-bs-custom-class="tooltip-expired" data-bs-title="Archive Record">
