                ...
                <div class="card-body m-2">
                    <div class="fs-3 fw-bold float-end">
                        <?php
                            ...
                            $sql = "SELECT COUNT(status) as 'total' FROM schedules WHERE scheduleType = 'medication' AND status = 'completed' AND archive = 'not archived' AND administeredBy = $employee_ID";
                            $stmt = $conn->query($sql);
                            $stmt->execute();
                    ...
                    <div>
                        <h5 class="card-subtitle text-muted fs-6"><span><strong> Completed <span class="text-dark"> Medication</span>  </strong>  Schedules:</h5>
                    </div>
                ...
                <div class="card-body m-2">
                    <div class="fs-3 fw-bold float-end">
                        <?php
                            ...
                            $sql = "SELECT COUNT(status) as 'total' FROM schedules WHERE scheduleType = 'medication' AND status = 'pending' AND archive = 'not archived' AND administeredBy = $employee_ID";
                            $stmt = $conn->query($sql);
                            $stmt->execute();
                            print_r($stmt->fetchColumn());
                    ...
                    <div>
                        <h5 class="card-subtitle text-muted fs-6"><span><strong> Pending <span class="text-dark">  &nbsp;Medication</span> </strong>Schedules: </h5>
                    </div>
                ...
                <div class="card-body m-2">
                    <div class="fs-3 fw-bold float-end">
                        <?php
                            ...
                            //to count how many schedules are completed
                            $sql = "SELECT COUNT(status) as 'total' FROM schedules WHERE scheduleType = 'vaccination' AND status = 'completed' AND archive = 'not archived' AND administeredBy = $employee_ID";
                            $stmt = $conn->query($sql);
                            $stmt->execute();
                            print_r($stmt->fetchColumn());
                ...
                <div class="card-body m-2">
                    <div class="fs-3 fw-bold float-end">
                        <?php
                            ...
                            //to count how many schedules are pending
                            $sql = "SELECT COUNT(status) as 'total' FROM schedules WHERE scheduleType = 'vaccination' AND status = 'pending' AND archive = 'not archived' AND administeredBy = $employee_ID";
                            $stmt = $conn->query($sql);
                            $stmt->execute();
                            print_r($stmt->fetchColumn());
